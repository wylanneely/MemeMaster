...
//  MemeMaster
//
//  Created by ALIA M NEELY on 8/8/17.
...
class BestMeme: Equatable {
    ...
    var textColorDataString: String {
        let colorData = NSKeyedArchiver.archivedData(withRootObject: textColor)
        guard let colorDataString = String(data: colorData, encoding: String.Encoding.utf8) else {return ""}
        return colorDataString
    }
    ...
    var cloudKitRecord: CKRecord {
        ...
        let record = CKRecord(recordType: BestMeme.cKRecordTypeKey, recordID: recordID)
        ...
        record.setValue(textColorDataString, forKey: kTextColor)
    ...
    init(){
        let record = CKRecord(recordType: "BestMeme")
    ...
    init?(cloudKitRecord: CKRecord){
        ...
        let textColorString = cloudKitRecord[kTextColor] as? String else { return nil }
        guard let recordID = UUID(uuidString: recordIDString) else { return nil }
        guard let colorData = NSData(base64Encoded: textColorString, options: .ignoreUnknownCharacters) as Data? else {return nil}
        guard let textColor = NSKeyedUnarchiver.unarchiveObject(with: colorData) as? UIColor else {return nil}
    ...
    //MARK: - CloudKit Keys
    static let cKRecordTypeKey = "BestMeme"
...
func ==(lhs: BestMeme, rhs: BestMeme) -> Bool {
